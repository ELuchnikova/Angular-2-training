...
export class WeatherService {
    ...
    public getWeatherDataByLocation(location: IDot) {
        return fetch(`http://api.openweathermap.org/data/2.5/weather?lat=${location.lat}&lon=${location.lng}&APPID=${WEATHER_API_KEY}`)
            .then((response: Response): Promise<ICityWeather> => response.json());
    }
}